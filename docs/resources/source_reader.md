...
- `status_override` (String) Override the source reader status after create/update. Currently only `paused` is supported. If set to `paused`, a shared source reader will not be deployed after creation or update.
