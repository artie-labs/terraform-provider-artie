...
- `status_override` (String) Only available if the source reader has `is_shared` set to true. This setting overrides the source reader status after create/update. Currently only `paused` is supported. If set to `paused`, a shared source reader will not be deployed after creation or update.
