...
resource "artie_pipeline" "postgres_to_snowflake" {
  ...
  # Optional: set to "paused" to create the pipeline without starting it
...
- `status_override` (String) Override the pipeline status after create/update. Currently only `paused` is supported. If set to `paused`, the pipeline will not be started after creation or update.
