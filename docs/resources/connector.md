...
resource "artie_connector" "databricks_pat" {
  name = "Databricks (PAT)"
  type = "databricks"
  databricks_config = {
...
resource "artie_connector" "databricks_oauth" {
  name = "Databricks (OAuth M2M)"
  type = "databricks"
  databricks_config = {
    host          = "my-workspace.cloud.databricks.com"
    http_path     = "/sql/1.0/warehouses/abc123"
...
Exactly one authentication method must be configured: either `personal_access_token` alone, or both `client_id` and `client_secret` together (OAuth M2M).
